...
def test_add_document(search_engine, mocker):
...
def test_search_and(search_engine, mocker):
...
def test_search_and_no_results(search_engine, mocker):
...
def test_search_or(search_engine, mocker):
...
def test_search_or_no_results(search_engine, mocker):
    search_engine.indexer.get_ids.side_effect = lambda word: []
...
def test_search_not(search_engine, mocker):
...
def test_search_and_with_ranking(search_engine, mocker):
    search_engine.indexer.get_ids.side_effect = lambda word: [1] if word == "test" else [1, 2]
...
def test_remove_document(search_engine, mocker):
    doc_id = 1
